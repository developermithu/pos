<div x-data="{ bannerVisible: false }" x-show="bannerVisible" x-transition:enter="transition ease-out duration-500"
    ...
    <div class="flex items-center justify-between w-full h-full px-3 mx-auto max-w-7xl ">
        <a
            ...
            <span class="flex items-center animate-pulse">
                <svg class="w-4 h-4 mr-1 font-bold" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <strong class="font-bold"> {{ __('New Update') }} </strong>
                <span class="hidden w-px h-4 mx-3 rounded-full bg-black/30 sm:block"></span>
            </span>
            <span class="block pt-1 pb-2 leading-none sm:inline sm:pt-0 sm:pb-0">
                {{ __('Application has been updated.') }}
            </span>
        </a>

        <button x-on:click="bannerVisible=false;"
            class="flex items-center flex-shrink-0 translate-x-1 ease-out duration-150 justify-center w-6 h-6 p-1.5 text-black rounded-full hover:bg-primary/40">
