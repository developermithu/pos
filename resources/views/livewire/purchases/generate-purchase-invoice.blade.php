            ...
            <div
                ...
                <div class="flex flex-wrap items-start justify-between gap-4">
                    <h1 class="order-2 text-lg font-semibold uppercase sm:text-xl md:text-2xl md:order-1">
                        {{ __('invoice no') }} #{{ $purchase->invoice_no }}</h1>
                ...
                <div class="capitalize">
                    <h3 class="pb-2 text-xl font-bold"> {{ __('bill from') }} </h3>
                    ...
                    <x-table.row class="hover:bg-transparent dark:hover:bg-bg-transparent">
                        <td colspan="4" class="pt-4 text-gray-500">
                            <div class="flex flex-col gap-1 text-right">
                                <div> {{ __('subtotal') }}: <strong> {{ number_format($purchase->subtotal) }}
                                    </strong></div>
                                <div> {{ __('tax') }}: <strong> {{ number_format($purchase->tax ?? 0) }}
                                    </strong></div>
                                <div> {{ __('total') }}: <strong> {{ number_format($purchase->total) }} </strong>
                                </div>
                ...
                <div class="flex flex-wrap items-center justify-end gap-3 print:hidden">
                    <x-button type="secondary" :href="route('admin.purchases.create')">
                        {{ __('go back') }} </x-button>
